...
class sononet_grid_attention_v2(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=3, in_channels=2, is_batchnorm=True, n_convs=None,
        ...
        super(sononet_grid_attention_v2, self).__init__()
        ...
        self.compatibility_score2 = AttentionBlock3D(in_channels=filters[3], gating_channels=filters[3],
                                                     inter_channels=filters[3], sub_sample_factor=(1,1,1),
                                                     mode=nonlocal_mode)
        self.compatibility_score3 = AttentionBlock3D(in_channels=filters[1], gating_channels=filters[3],
                                                     inter_channels=filters[3], sub_sample_factor=(1,1,1),
                                                     mode=nonlocal_mode)
        ...
        self.attention_filter_sizes = [filters[2], filters[3],filters[1], filters[0]]

        if aggregation_mode == 'concat':
            # self.classifier = nn.Linear(filters[2]+filters[3]+filters[1]+filters[0]+filters[3], n_classes)
    ...
    def forward(self, inputs):
        ...
        # g_conv3, att3 = self.compatibility_score3(conv2, conv5)
        # g_conv4, att4 = self.compatibility_score4(conv1, conv5)
        ...
        # g4 = torch.sum(g_conv4.view(batch_size, fsizes[3], -1), dim=-1)
